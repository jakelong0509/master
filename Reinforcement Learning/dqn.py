...
class DQN:
    def __init__(self, D, K, layer_size, gamma, max_experience = 1000000, min_experience = 50000, history_length = 4, batch_size = 32):
        self.D = D
        self.K = K
        self.layers = []
        # Create hidden layer
        ...
        self.cost = cost
        self.train_op = tf.train.RMSPropOptimizer(0.00025, decay=0.99, momentum = 0.0, epsilon = 0.01).minimize(cost)
...
def play_one_epi(env, tmodel, qmodel, gamma, eps, iters, initial_action, initial_observations, update_frequency = 10000):
    totalreward = 0
    ...
    while not done:
        ...
        # Skip frame
        if iters % 4 == 0:
            action = qmodel.sample_action(observations, eps)
    ...
    return totalreward, iters, action, observations
...
def main():
    ...
    for t in range(N):
        ...
        totalreward, iters, initial_action, initial_observations = play_one_epi(env, tmodel, qmodel, gamma, epsilon, iters, initial_action, initial_observations)
